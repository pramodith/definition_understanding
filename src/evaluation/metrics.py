...
def extract_predicted_word(response: str) -> str:
    ...
    phrase_matches = [
        re.search(r'(?:word is|answer is|word would be|answer would be)\s+["\']?([a-zA-Z]+)["\']?', response, re.IGNORECASE),
        re.search(r'(?:I think the word is|I believe the word is)\s+["\']?([a-zA-Z]+)["\']?', response, re.IGNORECASE)
    ]
...
def calculate_metrics(
    ...
    exact_matches = [normalize_word(pred) == normalize_word(true) for pred, true in zip(y_pred, y_true)]
...
def analyze_results_by_category(
    ...
    for category in df['length_category'].unique():
        ...
        if category_results:
            length_metrics[str(category)] = calculate_metrics(category_results, include_synonyms)
    
    return {
        'by_part_of_speech': pos_metrics,
        'by_word_length': length_metrics
    }
