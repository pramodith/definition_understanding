...
from evaluation.metrics import (calculate_metrics, analyze_results_by_category,
                                   extract_predicted_word, save_evaluation_results)
from models.model_factory import get_model


def create_prompt(definition: str, part_of_speech: Optional[str] = None) -> str:
    """
    ...
    system_prompt = "You are a word prediction model." \
    ...
    instructions = (
        f"Respond with just the word and no additional text."
        f"# Examples:\n"
        f"Definition: A custom-made or tailored item.\nPart of speech: adjective\n"
        f"Bespoke\n"
        f"Definition: To rage in excess of.\nPart of speech: verb\n"
        f"Outrage\n"
    )

    system_prompt = system_prompt + "\n\n" + instructions
    user_query = f"Definition: {definition}\nPart of speech: {pos_info}\n"
    prompt_message = [{"role": "system", "content": system_prompt}, {"role": "user", "content": user_query}]
...
async def evaluate_model_async(
    ...
    print(f"Evaluating {model_name} on {len(df)} samples using async batch processing...")
    ...
    for i, response in enumerate(responses):
        ...
        if verbose:
            ...
            print(f"Correct: {prediction.lower() == words[i].lower() or prediction.lower() in [s.lower() for s in all_synonyms[i]]}")
            print("-" * 50)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate LLM performance on word definition understanding")
    parser.add_argument("--model", type=str, required=False, help="Name of the model to evaluate", default="gpt-4o-mini")
    parser.add_argument("--dataset", type=str, required=False, 
                        help="Path to the dataset of word definitions", default="data/processed_dictionary.csv")
    parser.add_argument("--output-dir", type=str, required=False, 
                        help="Directory to save evaluation results", default="results")
    parser.add_argument("--num-samples", type=int, required=False, help="Number of samples to evaluate (None for all)", default=10)
    parser.add_argument("--no-synonyms", action="store_true", help="Don't consider synonyms as correct answers")
    parser.add_argument("--fuzzy-match", action="store_true", help="Allow fuzzy matching")
    parser.add_argument("--verbose", action="store_true", help="Print detailed information")
    parser.add_argument("--batch-size", type=int, default=20, help="Number of prompts to process in each batch")
